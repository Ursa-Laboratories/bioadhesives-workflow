...
# Device endpoints used by the automated workflow.
...
# The automated CLI starts and stops this bundled arm server automatically.
ARM_SERVER_PYTHON = sys.executable
ARM_SERVER_HOST = "127.0.0.1"
ARM_SERVER_PORT = 5004
ARM_SERVER_BASE_URL = f"http://{ARM_SERVER_HOST}:{ARM_SERVER_PORT}"
ARM_SERVER_STARTUP_TIMEOUT_S = 15.0
ARM_TIMEOUT_S = 300.0
MOCK_ARM = False

# xArm + Vention hardware settings used by automated_bioadhesives_workcell.arm_server_app.
ARM_IP = "10.210.29.16"
RAIL_IP = "10.210.29.15"
ARM_SPEED = 50
RAIL_TIMEOUT = 15

# Arm poses are [x, y, z, roll, pitch, yaw] in the xArm base frame (mm / deg).
...
ASMI_SLIDE_IN_PUSH = [270.5, 42, 33.5, 180, 0, 90]
...
# Opentrons deck layout and pipetting settings.
...
class AutomatedWorkflowSettings:
    ...
    arm_base_url: str = ARM_SERVER_BASE_URL
...
def build_workflow(
    ...
    *,
    ...
    experiment.raw["workflow"] = "automated_bioadhesives_workcell"
    opentrons_client = (
    ...
    runners = AutomatedRunners(
        ...
        arm=ArmTransferClient(
            settings.arm_base_url,
            timeout_s=float(settings.arm_timeout_s),
...
def ot_positions(plate_type: str = OT_PLATE_TYPE):
    positions = OT_TRANSPARENT if plate_type == "transparent" else OT_BLACK
    return positions["D1_PICKUP"], positions["D1_LIFTED"]
